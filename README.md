...
`packages.txt` installs `libgl1` and `libglib2.0-0t64` for the Streamlit Community Cloud deploy. It exists because `rapidocr-onnxruntime` depends on the GUI `opencv-python` distribution, which shares its `cv2` files with the headless wheel pinned in `requirements.txt` and wins on import — so `import cv2` asks for `libGL.so.1` regardless of the pin. The `Dockerfile` solves this at the source by force-reinstalling the headless wheel last; Streamlit Cloud has no equivalent hook, so the system library is installed instead.
...
- **Keep it minimal.** `apt` installs the file as one transaction, so a single unsatisfiable package prevents *all* of them from installing. Glib is the live example: the 2026-08-03 rebuild moved Community Cloud onto a Debian trixie base that no longer preinstalls it, so `import cv2` began failing on `libgthread-2.0.so.0` — but the environment's apt still carries a Debian 11 security index in which `libglib2.0-0` is a real candidate with uninstallable dependencies (`libffi7`, `libpcre3`), so requesting it by that name kills the whole transaction, `libgl1` included. The versioned trixie name `libglib2.0-0t64` must be pinned instead; it exists only in the trixie index and resolves cleanly (verified against `debian:trixie-slim` with the mixed bullseye-security index reproduced).
