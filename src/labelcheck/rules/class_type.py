...
def verify(
    ...
    *,
    ...
    if confidence >= FUZZY_PASS_THRESHOLD:
        ...
        detail = f"Class/type similarity {confidence:.1f} is ambiguous and needs agent review."
    else:
        status = Status.FAIL
        detail = f"Class/type similarity {confidence:.1f} is below the accepted bands."
