...
SEARCH_FORM_ERROR = (
    "TTB Public COLA Registry search is currently unavailable or its search form has "
    "changed; retry later, or bypass search with --ttbid ############## "
    "(repeat --ttbid for multiple 14-digit TTB IDs)"
)
...
class _SearchFormParser(HTMLParser):
    ...
    def form(self) -> _SearchForm:
        if not self.method:
            raise _SearchError(SEARCH_FORM_ERROR)
...
def _search_fields(
    ...
    product: str,
) -> tuple[_SearchForm, list[tuple[str, str]]]:
...
def _search_action_url(form: _SearchForm) -> str:
    action_url = urljoin(SEARCH_PAGE_URL, form.action)
    action = urlparse(action_url)
    search_page = urlparse(SEARCH_PAGE_URL)
    expected_action = urlparse(urljoin(BASE_URL, "publicSearchColasBasicProcess.do"))
    if (
        not form.action
        or action.scheme != "https"
        or action.netloc.casefold() != search_page.netloc.casefold()
        or action.path.casefold() != expected_action.path.casefold()
        or parse_qs(action.query).get("action") != ["search"]
    ):
        raise _SearchError(SEARCH_FORM_ERROR)
    return action_url
...
def _search_ttbids(client: _PoliteClient, search_page_html: str, product: str) -> list[str]:
    form, fields = _search_fields(search_page_html, product)
    response = client.post(_search_action_url(form), fields)
...
def _labeled_value(line: str, label: str) -> str | None:
    match = re.fullmatch(rf"{re.escape(label)}\s*(.*)", line)
    return None if match is None else match.group(1)


def _line_starts_with_label(line: str) -> bool:
    return any(_labeled_value(line, label) is not None for label in DETAIL_LABELS)


def _field_value(lines: Sequence[str], label: str) -> str:
    for line in lines:
        value = _labeled_value(line, label)
...
def _plant_address(lines: Sequence[str]) -> str:
    ...
    for line in lines:
        if not collecting:
            value = _labeled_value(line, PLANT_LABEL)
            if value is None:
                continue
            collecting = True
            if value:
...
def _attachment_urls(printable_html: str) -> list[str]:
    ...
    urls: list[str] = []
    registry = urlparse(BASE_URL)
    for source in parser.image_sources:
        absolute = urljoin(BASE_URL, source)
        parsed = urlparse(absolute)
        if (
            parsed.scheme != "https"
            or parsed.netloc.casefold() != registry.netloc.casefold()
            or not parsed.path.endswith("/publicViewAttachment.do")
        ):
            continue
    ...
    *,
    ...
    if explicit_ttbids:
        ...
        try:
            ...
            return [], stats, SEARCH_FORM_ERROR
